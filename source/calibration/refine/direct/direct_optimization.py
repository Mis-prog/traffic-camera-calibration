...
class DirectProjectionOptimizer:
    ...
    def error_line(self, line_known: tuple[PointND, PointND],
                   line_predicted: tuple[PointND, PointND]) -> float:
        known_start, known_end = line_known
        predicted_start, predicted_end = line_predicted
    ...
    def residuals_projection(self, params: np.ndarray,
                             lines: list[tuple[tuple[PointND, PointND], tuple[PointND, PointND]]]) -> np.ndarray:
        residuals = []
        ...
        for known_start, known_end in lines:
            ...
            predicted_start_2D = self.camera.direct(known_start_3D, params)
            predicted_end_2D = self.camera.direct(known_end_3D, params)
    ...
    def optimize_init(self, lines: list[tuple[tuple[PointND, PointND], tuple[PointND, PointND]]]):
        ...
        result = least_squares(self.residuals_projection, x0, args=(lines,), method='trf')
        return self.camera, result

    def optimize_reprojection(self, lines: list[tuple[tuple[PointND, PointND], tuple[PointND, PointND]]]):
        angles = self.camera.get_R(angle_output=True)
        ...
        def wrapped_residuals(params):
            residuals = self.residuals_projection(params, lines)
